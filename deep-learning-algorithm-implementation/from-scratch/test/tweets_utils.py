...
def get_data(train_size: int = 4500):
    ...
    freq_pos.update(freq_neg)

    word_freq = freq_pos

    return train_tweets, train_labels, test_tweets, test_labels, word_freq


def build_dictionary(
    word_freq, max_token_num=None, eos_token: str = "<eos>", unk_token: str = "<unk>"
):
    if max_token_num is not None:
        word_freq = word_freq.most_common(max_token_num)

    word_freq.extend([(eos_token, 1), (unk_token, 1)])
    tokens = sorted(map(lambda item: item[0], word_freq))
    token_dictionary = dict(zip(tokens, range(1, 1 + len(tokens))))
...
def encode_tweets(
    ...
    unknown_id = token_dictionary[unk_token]

    for i, tweet in enumerate(tweets):
        tweet.append(eos_token)

        for j, token in enumerate(tweet):
            tweets[i][j] = token_dictionary.get(token, unknown_id)
