...
def fit(
    ...
    while it < max_it and not np.isnan(prev_mse):
        ...
        X_cur = X[inst_id:inst_id+batch_size+1, :]
        y_cur = y[inst_id:inst_id+batch_size+1]

        # Note: not 100% correct, since it fully works only when the minibatch
        # size is a multiple of the number of instances!
        inst_id = (inst_id + batch_size) % y.size

        y_pred = predict(X_cur, theta, add_bias=False)
        err = y_pred - y_cur
        theta -= step_size * np.mean(err * X_cur.T, axis=1)
...
def _test():
    ...
    X1 = np.arange(m) + 8 * np.random.randn(m)
    ...
    if dims == 2:
        ...
        X = X1
        y = 0.8 * X1 + 16 * np.random.randn(m)

    X = 2 * (X - X.min(axis=0)) / np.ptp(X, axis=0) - 1

    rand_inds = np.arange(y.size)
