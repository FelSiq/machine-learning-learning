...
class adaboost:
    ...
    def _updateWeight(self, weight, error, misclassified, ep=1.0e-7):
        return weight * 0.5 / (ep + (error if misclassified else
                                     (1.0 - error)))
    ...
    def ensemble(self, instNum, misclassified, maxIteration=10, verbose=False):
        ...
        while it < maxIteration:
            ...
            if verbose:
                print('Round', it, '\tchoosen classifier:', bestClassifier,
                      '\terror:', minError)
            alphas[bestClassifier] += self._calcAlpha(minError)

            # Update weights
            for n in range(instNum):
                weights[n] = self._updateWeight(
                    weights[n], minError, n in misclassified[bestClassifier])
...
if __name__ == '__main__':
    ...
    strongerClassifier = adaboost().ensemble(
        instNum, misclassified, verbose=True, maxIteration=10)
