...
class Pso():
    def __init__(self, num_particles=100, interval=(0, 1), dimension=2):
    ...
    def renew(self, num_particles=100, interval=(0, 1), dimension=2):
    ...
    def run(self,
        ...
        if not self.goal or len(goal) < len(self.goal) or any(
                goal != self.goal):
            ...
            # the goal
            self.particle_cost = self.__costfunction__(
        ...
        while it_num < iteration_max and break_counter > 0:
            ...
            if print_progress and (it_num % it_per_print == 0):
                print(it_num, ":", self.global_best_position, "(variance:",
                      best_to_global_var, ")")
