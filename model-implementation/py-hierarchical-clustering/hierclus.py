...
from kmeans2 import Kmeans  # noqa: E402
...
class Hierclus():
    ...
    def run(dataset,
        ...
        while num_supergroups > min_cluster_num:
            ...
            nearest_pair = min(aux_dist, key=lambda k: k["clustering_metric"])
    ...
    def fit(dataset, min_cluster_num, k, plot=True):
        ...
        inst_cluster_id = Kmeans.run(dataset=dataset, k=k)["inst_cluster_id"]
        ...
        if plot:
            if dataset.shape[1] == 2:
                ...
                plt.scatter(x=dataset[:, 0], y=dataset[:, 1], c=plot_colors)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print(
            "usage: " + sys.argv[0] + " <data_filepath> <linkage_type>",
            "\t[-label class_label]",
    ...
    if fitdata:
        ans = Hierclus.fit(dataset.iloc[:, :].values, min_cluster_num=n, k=k)
    else:
        ans = Hierclus.run(
            dataset.iloc[:, :].values,
