...
class mlp:
    ...
    def _adjustWeights(self, x, y, stepSize, Lambda=1.0):
        ...
        for k in range(self.outputLayerSize):
            delta_o.append(
                error[k] * self._d_sigmoid(self.outputNet[k], Lambda))

        delta_h = []
        for j in range(self.hiddenLayerSize):
            aux = np.sum([
                delta_o[k] * self.wOutput[k, j]
                for k in range(self.outputLayerSize)
            ])
            delta_h.append(self._d_sigmoid(self.hiddenNet[j], Lambda) * aux)

        for k in range(self.outputLayerSize):
            self.wOutput[k] += stepSize * delta_o[k] * np.concatenate(
                (self.hiddenLayerOutput, [1.0]))

        for j in range(self.hiddenLayerSize):
            self.wHidden[j] += stepSize * delta_h[j] * np.concatenate(
                (x, [1.0]))
    ...
    def predict(self, query, Lambda=1.0):
        self.hiddenNet = np.array(
            [np.sum(j * np.concatenate((query, [1.0]))) for j in self.wHidden])
        self.hiddenLayerOutput = np.array(
            [self._sigmoid(i, Lambda) for i in self.hiddenNet])

        self.outputNet = np.array([
            np.sum(j * np.concatenate((self.hiddenLayerOutput, [1.0])))
            for j in self.wOutput
        ])
        self.predictOutput = np.array(
            [self._sigmoid(j, Lambda) for j in self.outputNet])
    ...
    def _initWeights(self):
        self.wHidden = np.array(
            [[random.random() - 0.5 for __ in range(self.inputLayerSize + 1)]
             for _ in range(self.hiddenLayerSize)])
        self.wOutput = np.array(
            [[random.random() - 0.5 for __ in range(self.hiddenLayerSize + 1)]
    ...
    def fit(self,
        ...
        while meanSqrError > maxError and curIteration < maxIteration:
            ...
            for i in range(n):
                meanSqrError += self._adjustWeights(x[i], y[i], stepSize,
                                                    Lambda)
            meanSqrError /= n
            ...
            if (showError):
                print('I:', '{value:<{fill}}'.format(
                    value=curIteration, fill=15), 'MSE:', meanSqrError)
    ...
    def plot(self, x, y, dim=250):
        ...
        for sample in matpoints:
            classifications.append(
                classesColors[int(self.predict(sample) >= 0.5)])
