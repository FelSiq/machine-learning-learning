...
class Mlp:
    ...
    def build(self, sizes=(2, 2, 2)):
        ...
        self.weights = {
            'hidden':
            np.array([np.random.rand(n + 1) - 0.5 for i in range(m)]),
            'output': np.array([np.random.rand(m + 1) - 0.5 for i in range(k)])
        }
    ...
    def sig(self, netVector):
		...
		"""
        try:
            return np.array([1.0 / (1.0 + np.exp(-x)) for x in netVector])
        except:
    ...
    def foward(self, x, retComplete=False):
		...
		"""
        hnet = np.dot(self.weights['hidden'], np.concatenate((x, [1.0])))
        fhnet = self.sig(hnet)

        onet = np.dot(self.weights['output'], np.concatenate((fhnet, [1.0])))
        fonet = self.sig(onet)
    ...
    def backward(self, fonet, onet, fhnet, hnet, itErr, x, eta=0.1):
        ...
        outputAdjust = np.concatenate((self.sig(hnet), [1.0])) * deltaOut
        """
        ...
        for i in range(hiddenLayerSize):
            auxSum = (np.dot(deltaOut, self.weights['output'][:, i]) *
    ...
    def train(self, x, y, m=2, maxError=1.0e-3, maxIt=1e+4):
        ...
        while meanSqrError > maxError and i < maxIt:
            ...
            for inst, lab in zip(x, y):
                fonet, onet, fhnet, hnet = self.foward(inst, retComplete=True)
                """
