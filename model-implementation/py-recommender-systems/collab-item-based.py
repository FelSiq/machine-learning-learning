...
class Ibcf:
    ...
    def fit(self, x, y, neighborhood_size=3):
        ...
        sqr_item_count = np.array(
            [sum(y == item) for item in item_labels])**0.5
        ...
        for item_a_index in range(item_count - 1):
            ...
            for item_b_index in range(item_a_index + 1, item_count):
                ...
                cur_sim_val = len(
                    users_of_item_a.intersection(users_of_item_b))
        ...
        self.sim_matrix = DataFrame(
            self.sim_matrix, columns=item_labels, index=item_labels)
...
if __name__ == "__main__":
    ...
    data = read_csv(sys.argv[1], sep=sep, names=["Users", "Items"])

    model = Ibcf(
        data.iloc[:, 0].values, data.iloc[:, 1].values, neighborhood_size=k)
