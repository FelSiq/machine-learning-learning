...
class Pca:
    ...
    def transform(self, filepath, normalize=True, sep=",", plot=False):
        ...
        dataset = pd.DataFrame(
            np.matmul(dataset, self.eigenvectors), columns=dataset.columns)
    ...
    def feature_selection(self, dataset, eigenvalues=None, information=0.95):
        ...
        new_dataset = pd.DataFrame.copy(dataset).drop(
            dataset.columns[del_indexes], axis=1)
...
if __name__ == "__main__":
    ...
    # basis
    t1_dataset = model.transform(filepath=sys.argv[1], normalize=normalize)
